/**
 * Database Seed Data File
 * 
 * Purpose:
 * - Provides initial data structure for categories and tasks in the M&M Consultations platform
 * - Used by scripts/seedDatabase.js to populate MongoDB with default content
 * - Defines complete relocation checklist for Indian professionals moving to Germany
 * 
 * Data Structure:
 * - Categories: 4 phases of relocation journey (beforeArrival, uponArrival, firstWeeks, ongoing)
 * - Tasks: 21 detailed tasks with rich HTML descriptions, tips, requirements, and external links
 * 
 * Categories:
 * 1. Before Arrival (2-3 months before) - Blue (#3B82F6) - 4 tasks
 * 2. Upon Arrival (first week) - Green (#10B981) - 6 tasks
 * 3. First Weeks (first month) - Amber (#F59E0B) - 5 tasks
 * 4. Ongoing (throughout stay) - Purple (#8B5CF6) - 5 tasks
 * 
 * Task Properties:
 * - id: Unique identifier (kebab-case)
 * - title: Display name for users
 * - description: Rich HTML content with formatting, lists, headings
 * - category: References category id
 * - order: Display sequence within category
 * - estimatedDuration: Time commitment required
 * - difficulty: easy/medium/hard
 * - externalLinks: Array of helpful resources with URLs
 * - tips: Array of practical advice strings
 * - requirements: Array of documents/items needed
 * 
 * Integration Points:
 * - M&M Consultants services prominently linked (airport pickup, SIM card, orientation events)
 * - Official German websites and forms referenced
 * - Popular third-party services recommended (TK insurance, Skyscanner, WG-Gesucht)
 * 
 * Content Features:
 * - Culturally relevant for Indian → Germany relocation
 * - Comprehensive coverage of bureaucratic requirements (Anmeldung, visa, insurance)
 * - Practical advice on daily life (transportation, banking, healthcare)
 * - Social integration guidance (student groups, cultural events, networking)
 * - Financial planning and work regulations
 * 
 * Usage:
 * - Run `node scripts/seedDatabase.js` to populate database
 * - Categories and tasks displayed in user dashboard checklist
 * - Admin can manage via /dashboard/admin interface
 */
...
/**
 * Main seed data object containing categories and tasks for database initialization
 * 
 * Structure:
 * - categories: Array of 4 relocation phase categories
 * - tasks: Array of 21 detailed relocation tasks
 * 
 * Each category includes:
 * - id, name, displayName: Identification fields
 * - description: Brief explanation of phase
 * - icon: Lucide icon identifier
 * - color: Hex color for UI theming
 * - order: Display sequence
 * - estimatedTimeFrame: When this phase occurs
 * 
 * Each task includes:
 * - id, title: Identification
 * - description: Comprehensive HTML guide (lists, headings, paragraphs)
 * - category: Phase reference
 * - order: Sequence within category
 * - estimatedDuration: Time to complete
 * - difficulty: Complexity level
 * - externalLinks: Helpful resources (optional)
 * - tips: Practical advice (optional)
...
const seedData = {
   ...
   * Categories Array
   * 
   * Defines the 4 main phases of the Germany relocation journey:
   * 1. Before Arrival - Pre-departure preparations
   * 2. Upon Arrival - First week essentials
   * 3. First Weeks - Initial settlement tasks
   * 4. Ongoing - Continuous maintenance activities
   * 
   * Each category has unique color coding and icon for visual distinction
   */
  categories: [
    {
      id: "beforeArrival",
   ...
   * Tasks Array
   * 
   * Comprehensive list of 21 relocation tasks organized by category:
   * 
   * Before Arrival (4 tasks):
   * - find-accommodation: Housing search strategy
   * - health-insurance: TK insurance setup
   * - book-flights: Flight booking and airport selection
   * - prepare-documents: Document preparation and translation
   * 
   * Upon Arrival (6 tasks):
   * - airport-pickup: M&M Consultants pickup service
   * - settle-accommodation: Move-in checklist
   * - anmeldung: Mandatory residence registration
   * - open-bank-account: German banking setup
   * - get-sim-card: Mobile connectivity
   * - public-transportation: Transit system orientation
   * 
   * First Weeks (5 tasks):
   * - orientation-events: M&M Consultants integration events
   * - university-registration: Student enrollment
   * - get-student-id: Campus access and discounts
   * - explore-city: Local area familiarization
   * - join-student-groups: Social integration
   * 
   * Ongoing (5 tasks):
   * - manage-finances: Budgeting and money management
   * - maintain-visa-status: Legal status maintenance
   * - seek-part-time-work: Employment opportunities
   * - access-healthcare: Medical system navigation
   * - cultural-events: Cultural participation
   * 
   * Each task features:
   * - Rich HTML descriptions with proper formatting
   * - Bulleted/numbered lists for clarity
  ...
  tasks: [
    // =====================================================
    // BEFORE ARRIVAL TASKS (Category 1)
    // =====================================================
    {
